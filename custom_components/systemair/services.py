...
async def async_setup_services(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    try:
        ...
        async def async_handle_set_user_mode(call: ServiceCall) -> None:
            ...
            # Get the device ID from the entity ID
            entity_registry = async_get_entity_registry(hass)
            entity_entry = entity_registry.async_get(entity_id)
            if entity_entry is None or entity_entry.device_id is None:
                _LOGGER.error(f"Entity {entity_id} not found or has no device ID")
                return
                
            # Find the unit ID that corresponds to this device ID
            for unit_id, unit in coordinator.units.items():
                if f"{(DOMAIN, unit_id)}" == str(list(entity_entry.device_info.get("identifiers", [["", ""]]))[0]):
                    try:
